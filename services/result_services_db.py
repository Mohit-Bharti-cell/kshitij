...
def get_candidate_all_results(email: str):
    ...
    db = _mongo_db()
    cand_col = db['candidates']
    cand_docs = list(cand_col.find({'email': {'$regex': f'^{email}$', '$options': 'i'}}))
    jd_ids = []
    for c in cand_docs:
        jid = c.get('jdId')
        if jid:
            jd_ids.append(str(jid))
    question_sets_res = supabase.table('question_sets').select('id, jd_id').execute()
    question_sets_data = question_sets_res.data or []
    qs_to_jd_map = {qs['id']: qs['jd_id'] for qs in question_sets_data}

    if jd_ids:
        relevant_qs_ids = [qs_id for qs_id, jd in qs_to_jd_map.items() if jd in jd_ids]
    else:
        relevant_qs_ids = list(qs_to_jd_map.keys())

    results = []
    if relevant_qs_ids:
        res = supabase.table('test_results').select('*').eq('candidate_email', email).in_('question_set_id', relevant_qs_ids).execute()
        results = res.data or []
    ...
    if jd_ids:
        ...
        for jid in jd_ids:
            try:
                obj_ids.append(ObjectId(jid))
            except Exception:
                pass
        if obj_ids:
            for jd in jds_col.find({'_id': {'$in': obj_ids}}, {'_id':1, 'title':1}):
                title_map[str(jd.get('_id'))] = jd.get('title')

    out = []
    for r in results:
        qs_id = r.get('question_set_id')
        jd_id = qs_to_jd_map.get(qs_id)
        jd_title = title_map.get(str(jd_id)) if jd_id else None
        out.append({
            ...
            'status': r.get('status'),
            'tab switches': r.get('tab_switches'),
            'text selections': r.get('text_selections'),
            'copies': r.get('copies'),
            'pastes': r.get('pastes'),
            'right clicks': r.get('right_clicks'),
            'face not visible': r.get('face_not_visible'),
            'inactivities': r.get('inactivities'),
            'evaluated_at': r.get('evaluated_at') or r.get('created_at') or r.get('inserted_at')
        })
    if not out and cand_docs:
        for c in cand_docs:
            out.append({
                'name': c.get('name'),
                'email': c.get('email'),
                'jd_id': str(c.get('jdId')) if c.get('jdId') else None,
                'jobTitle': None,
                'score': c.get('score'),
                'evaluated_at': c.get('updatedAt') or c.get('createdAt')
            })
    return out
